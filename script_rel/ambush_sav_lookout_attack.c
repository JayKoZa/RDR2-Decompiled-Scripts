	...
	while (iVar0 <= (uParam0->f_215 - 1))
	{
		...
		else
		{
			...
			else
			{
				...
				PED::_SET_PED_CROUCH_MOVEMENT(uParam0[iVar0], 0, 0, 0);
		...
		while (iVar28 < 4)
		{
			...
			if (func_421(bParam0, &Var0, iVar5, 0))
			{
				...
				if (func_425(0))
				{
					...
					func_426(WEAPON::_GET_AMMO_TYPE_FOR_WEAPON(bParam0), iParam1, iParam6);
...
void func_427(bool bParam0, int iParam1)
{
	...
	{
		iVar1 = func_380(WEAPON::_GET_WEAPON_CLIP_SIZE(bParam0), iParam1);
...
void func_601(var uParam0)
{
	if (NETWORK::NETWORK_IS_GAME_IN_PROGRESS())
	{
		MAP::_SET_BLIP_NAME_FROM_PLAYER_STRING(*uParam0, func_672(func_671(255), joaat("COLOR_PURE_WHITE")));
	}
	else if (func_637())
	{
